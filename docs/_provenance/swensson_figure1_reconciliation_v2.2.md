...
**Supersedes:** v2.1 (two-source). Adds EDM's automated bar-detection digitization as third source.
...
3. **EDM** (`edm_digitization/`) — automated bar-detection with iterative debugging: anti-aliasing gap tolerance, IDKPTM-line gap-closing, $K+I \leq 5$ post-hoc rounding correction; reports zero-slack profile per child.

All three independently snap to integers in $[0, 5]$ and enforce protocol bound.

## Convergence on the load-bearing findings

| Quantity | CC-v2 | Ai1 | EDM | Verdict |
|---|---|---|---|---|
...
| Aden $K_t$ onset | s23 | s21 | s21 | Ai1+EDM agree at 21 |
...
| Aden break (asterisk position) | "around session 30" | not pixel-resolved | **between sessions 32–33** | EDM authoritative |

## EDM's added findings
...
**Reading:** Aden showed near-zero slack throughout intervention — clean stimulus control, every trial yielded a procedure-appropriate response (either trained mand or correct intraverbal). Alex and Ida had substantially more third-class responding. The EB carrier's saturation bound $K + I \leq 5$ admits a third quantity (the slack) which EDM's automated extraction surfaces as a behavioral signal the original analysis did not.

(Caveat: EDM's Alex sessions 53–62 show high slack readings that reflect bar-detection failure on the dense post-mastery part of the figure, not real third-class trials. The 39% zero-slack number is conservative within Alex's resolvable sessions.)
...
| Child | Stimulus-control onset (EDM) | Swensson mastery (4/5 sustained correct) |
|---|---:|---:|
...
### Break-pre vs break-post (EDM's reframing)

EDM places the break asterisk between sessions 32 and 33 (automated detection from figure pixel layout). Pre-break (sessions 30–32) IDKPTM = [2, 2, 4] = mean 2.67. Post-break (sessions 33–35) IDKPTM = [2, 2, 2] = mean 2.0.

**Caveat preserved by EDM:** *"Aden's IDKPTM had ALREADY been declining before the break as he learned answers — so the break is not a clean asymptote-vs-asymptote comparison. It is, however, a 6-week interruption of an active learning curve."*
...
| Issue | Magnitude |
|---|---|
| CC-v2 has Aden $K_t = 0$ through session 21; Ai1+EDM have $K_t = 2$ at session 21 | CC-v2 missed the K_t-onset session; aggregates absorb this |
| EDM Alex sessions 53–62 have unreadable bars (high slack) | Honest pixel-level limit; flagged in EDM debugging notes |
...
2. **Aden's acquisition transition (sessions 21–35)**: K-rail rises from 0 to 4–5, I-rail falls from 5 to 0–1, with $K + I \leq 5$ saturation throughout (89% of these sessions hit equality per EDM).
...
5. **Alex's high-variability profile**: slack rate (per EDM) shows Alex differs from Aden/Ida in stimulus-control quality, not just rate.
...
The narrow break-window comparison ($n=3$ each side). EDM's centre is small ($-0.67$) with hull straddling zero. The three-source disagreement on per-session $K_t$ in the transition phase makes any 3-session-window claim about the calendar break unreliable.
...
- They diverge on per-session integers in the transition phase (±1 typical) and on Alex's late-figure sessions (EDM honest limit).
- EDM's slack-as-third-rail and stimulus-control-onset findings are new EB carrier readings of the dataset that v2 and Ai1 do not surface.
...
- EDM: `edm_digitization/{digitized.csv, digitized.json, eb_findings.txt}`
